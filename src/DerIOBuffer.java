...
public class DerIOBuffer {
	...
	protected static Vector DER_Encode(MRSAPointF2m P) {
		int[] PC = new int[1];
		PC[0] = 0x04;
	...
	protected static MRSAPointF2m DER2Point(int[] der_rep) throws Exception {
		int length;
		if (der_rep[0] != 0x04)
			throw (new Exception("Not an Octet String"));
		else {
		...
		MRSAPointF2m p = new MRSAPointF2m(new F2m(x), new F2m(y));

		return p;
	}

	protected static int[] DER_Encode(MRSACurveF2m C) {
		int[] der_rep =
			Utils.concatenate(
	...
	protected static MRSACurveF2m DER2Curve(int[] der_rep) throws Exception {
		if (der_rep[0] != 0x30)
			throw (new Exception("Not a Sequence"));
		Vector v = DerSeqDecode(der_rep);
		return new MRSACurveF2m(
	...
	protected static int[] DER_Encode(MRSADomainParameters dp) {
		Vector v = new Vector();
		...
		v.insertElementAt(
			Utils.OS2V(DER_Encode(new MRSACurveF2m((F2m) dp.E.a4, (F2m) dp.E.a6))),
			v.size());
		// curve
		v.insertElementAt(DER_Encode((MRSAPointF2m) dp.G), v.size()); // base
	...
	protected static MRSADomainParameters DER2ECDP(int[] der_rep)
		throws Exception {
		if (der_rep[0] != 0x30)
		...
		MRSACurveF2m C = DER2Curve(Utils.Vector2OS((Vector) v.elementAt(2)));
		MRSAPointF2m G = DER2Point(Utils.Vector2OS((Vector) v.elementAt(3)));
		BigInteger r = DER2BigInt(Utils.Vector2OS((Vector) v.elementAt(4)));
		BigInteger k = DER2BigInt(Utils.Vector2OS((Vector) v.elementAt(5)));
		MRSADomainParameters dp;

		if (basis == 2)
			dp = new MRSADomainParameters(m, trinomial, C, r, G, k);
		else
			dp =
				new MRSADomainParameters(
					m,
					pentanomial_k3,
					pentanomial_k2,
					pentanomial_k1,
	...
	protected static int[] DER_Encode(MRSAPubKey pk) {
		...
		Vector ecpoint = DER_Encode((MRSAPointF2m) pk.W); // public key
	...
	protected static MRSAPubKey DER2ECPubKey(int[] der_rep) throws Exception {
		if (der_rep[0] != 0x30)
		...
		MRSAPointF2m W = DER2Point(Utils.Vector2OS((Vector) buf.elementAt(1)));

		MRSAPubKey pk = new MRSAPubKey(dp, W);
		return pk;
	}

	protected static int[] DER_Encode(MRSAPrivKey sk) {
		Vector v = new Vector();
		v.insertElementAt(DER_Encode(1), v.size()); // version
	...
	protected static MRSAPrivKey DER2ECPrivKey(int[] der_rep) throws Exception {
		if (der_rep[0] != 0x30)
			throw (new Exception("DER2ECPrivKey: Not a Sequence"));
		Vector v = DerSeqDecode(der_rep);
		if (DER2BigInt(Utils.Vector2OS((Vector) v.elementAt(0))).intValue()
		...
		MRSAPrivKey sk = new MRSAPrivKey(dp, s);
		return sk;
	}

	protected static int[] DER_Encode(MRSADSA sig) {
		Vector AlgorithmIdentifier = new Vector();
		// 06 Length (40*01+02+840+10045+02+01=)
	...
	protected static MRSADSA DER2ECSignature(int[] der_rep) throws Exception {
		if (der_rep[0] != 0x30)
		...
		return new MRSADSA(c, d);
	}

	protected static int[] DER_Encode(MRSAIES ct) {
		Vector buf = new Vector();
	...
	protected static MRSAIES DER2ECIES(int[] der_rep) throws Exception {
		if (der_rep[0] != 0x30)
			throw (new Exception("DER2ECIES: Not a Sequence"));

		Vector buf = DerSeqDecode(der_rep);
		MRSAPubKey V = DER2ECPubKey(Utils.Vector2OS((Vector) buf.elementAt(0)));
		int C[] = Utils.Vector2OS(DER2OCTETSTR((Vector) buf.elementAt(1)));
		int T[] = Utils.Vector2OS(DER2OCTETSTR((Vector) buf.elementAt(2)));
		return new MRSAIES(V, C, T);
	}

	/**
	 * Constructor
	 */
	public DerIOBuffer(MRSADomainParameters dp) {
		data = DER_Encode(dp);
	}

	public MRSADomainParameters toECDP() throws Exception {
	...
	public DerIOBuffer(MRSAPubKey pubKey) {
		data = DER_Encode(pubKey);
	}

	public MRSAPubKey toECPubKey() throws Exception {
	 ...
	 * Constructor
	 */
	public DerIOBuffer(MRSAPrivKey privKey) {
		data = DER_Encode(privKey);
	}

	public MRSAPrivKey toECPrivKey() throws Exception {
	 ...
	 * Constructor
	 */
	public DerIOBuffer(MRSADSA sig) {
		data = DER_Encode(sig);
	}

	public MRSADSA toECSignature() throws Exception {
	 ...
	 * Constructor
	 */
	public int[] DerIOBuffer(MRSAIES ct) {
		data = DER_Encode(ct);
...
return data;
	...
	public MRSAIES toECIES(int[] crypted) throws Exception
	{
